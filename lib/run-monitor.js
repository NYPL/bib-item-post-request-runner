...
class RunMonitor {
   ...
   */
  getStat (type, nyplSource, property) {
    if (this.stats && this.stats[type] && this.stats[type][nyplSource]) {
      return this.stats[type][nyplSource][property]
    }
  }

  /**
   *  Get an estimate of progress (expressed as a float between 0..1)
   *  for type/nyplSource pair
   */
  getProgress (type, nyplSource, processed) {
    const total = this.getStat(type, nyplSource, 'total')
    if (total) {
