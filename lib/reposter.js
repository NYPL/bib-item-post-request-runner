...
class Reposter {
  ...
  repost (which, nyplSource, options = {}, processedCount = 0) {
    ...
    if (options.limit && (typeof options.limit) !== 'number' || ((typeof options.limit) === 'number' && options.limit <= 0)) return Promise.reject(new Error('Invalid options.limit. Must be numeric'))
    ...
    const handleResponse = (resp) => {
      ...
      if (!resp || !resp.limit || (typeof resp.limit) !== 'number' || !resp.lastId) throw new UpstreamError(`Invalid response from ${path}: ${JSON.stringify(resp)}`)
      // Make sure the returned `lastId` represents forward progress:
      if (resp.lastId <= options.start) throw new UpstreamError(`Response from ${path} gave non-incrementing lastId: ${resp.lastId}`)
      ...
      let progressRatio = this.runMonitor.getProgress(which, params.nyplSource, resp.lastId, processedCount)
      let progressLabel = ''
      if (progressRatio) progressLabel = `, progress: ${(progressRatio * 100).toFixed(2)}%`

      logger.info(`Reposter: Processed ${processedCount} of ${options.limit || '[all]'}${progressLabel}`, { processedCount, type: which, nyplSource, options })
      ...
      // Handle firstId seek:
      } else if (options.limit === 1 && !params.lastId) {
  ...
  doPost (path, params, retryCount = 0) {
    return this.apiClient.post(path, params)
      ...
      .catch((e) => {
        ...
        logger.error(`Reposter: Error posting to ${path}, retry ${retryCount}`, { params })
        ...
        if (retryCount < MAX_RETRIES) {
          ...
          logger.debug(`Reposter: Waiting ${delaySeconds}s to rety with reduced limit of ${params.limit}`)
